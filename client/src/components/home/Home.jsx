    ...
    <h1 className="text-center">Our Gallery</h1>
      <div className="container relative">
        <div className="mySlides">
          ...
          <img src={imgArr[currImg]} className="w-full aspect-video object-cover" />
        </div>
      ...
      <div className="container">
        <p className="bg-[#222222] md:py-3 py-1 md:text-xl font-bold text-center text-white">{imgCap[currImg]}</p>
      </div>
      <div className="container grid md:grid-cols-7 grid-cols-3 mb-5">
        {imgArr.map((img, index) =>
          <div key={index} className="column">
            <img className="w-full aspect-video object-cover" onClick={() => setCurrImg(index)} src={img} alt="The Woods" />
          </div>
        )}
      </div>
    </>
